...
(facts "Saldo inicial é 0"
  ...
  (let [response (app (mock/request :get "/saldo"))]
    ...
    (fact "o texto do corpo é um JSON cuja 
          chave é saldo e o valor é 0"
...
(facts "Filtra transações por parâmetros de busca na URL"
  ...
  (against-background
    ...
    ;esse teste esta falando por causa do TODO
    ;no handler.clj
    (fact "Filtro sem rótulo"
      (let [response (app (mock/request
          :get "/transacoes?rotulos="))]
        (:status response) => 200
        (:body response) => (json/generate-string
          {:transacoes [livro curso salario]}))))